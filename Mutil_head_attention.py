...
train_df, val_df = train_test_split(train_df, test_size=0.99)
val_df, ff = train_test_split(train_df, test_size=0.99)
...
class Muti_head_Attention(object):
    ...
    def __init__(self, max_length, num_classes, hidden_num, num_blocks, num_heads):
        ...
        self.embedding_chars = self.input_x  # [None,sequence_length, embed_size]
        # projection to hidden_num
        self.embedding_chars = tf.layers.dense(self.embedding_chars, hidden_num, use_bias=True)
        ...
        self.accuracy = tf.reduce_mean(
            tf.cast(tf.equal(self.input_y, tf.cast(tf.argmax(self.score, axis=1), tf.int32)), tf.float32))
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        muti_head = Muti_head_Attention(max_length=SEQ_LEN,
                                        num_classes=2,
        ...
        for time in range(epoch):
            ...
            for trainX_batch, trainY_batch, text_length in tqdm(batch_gen_df(train_df)):
                feed_dict = {
                    muti_head.input_x: trainX_batch,
                    muti_head.input_y: trainY_batch,
            ...
            for valX_batch, valY_batch, text_length in tqdm(batch_gen_df(val_df)):
                feed_dict = {
                    muti_head.input_x: valX_batch,
                    muti_head.input_y: valY_batch,
                    muti_head.drop_out_prob: 1.0
                }
                dev_loss, dev_score = sess.run([muti_head.cost, muti_head.score], feed_dict)
        ...
        for x, length in tqdm(batch_gen_test(test_df)):
            ...
            y, s = sess.run([muti_head.predictions, muti_head.score], feed_dict)
